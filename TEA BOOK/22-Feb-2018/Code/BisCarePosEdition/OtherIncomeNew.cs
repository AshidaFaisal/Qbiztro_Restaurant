...
{
    public partial class OtherIncomeNew : Form
    {
        public OtherIncomeNew()
        {
            InitializeComponent();
        }
        ...
        private void OtherIncomeNew_Load(object sender, EventArgs e)
        {
            try
            {
                ApplyTheme();
            DataTable dt1 = new DataTable();
            dt1 = obj.GetFormUserRights(File.ReadAllText("user.txt"), "OtherIncome");
            ...
            DataSet ds1 = obj.GetOtherIncomeType();
                ...
                else
                {
                    ...
                    else
                    {
                        if (isupdate == 0)
                        {
                            if (SaveStatus == "1")
                            {
                                obj.InsertOtherIncome(dtp_newdate.Value, txt_amount.Text, cmb_newtype.Text, txt_description.Text);
                                MessageBox.Show("Other Income successfully saved", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                clear();
                                cmb_newtype.Focus();
                            }
                            else
                            {
                                ...
                                MessageBox.Show("You Do Not Have The Permission To Save Other Income", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        ...
                        else
                        {
                            if (isupdate == 1)
                            {
                                if (UpdateStatus == "1")
                                {
                                    obj.UpdateOtherIncome(dtp_newdate.Value, txt_amount.Text, cmb_newtype.Text, txt_description.Text, id);
                                    MessageBox.Show("Other Income details successfully updated", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    isupdate = 0;
                                    clear();
                                    cmb_newtype.Focus();
                                }
                                else
                                {
                                    ...
                                    MessageBox.Show("You Do Not Have The Permission To Update Other Income", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void bttn_search_Click(object sender, EventArgs e)
        {
            ...
            {
                dt = obj.SearchOtherIncome(start, end, cmb_edittype.Text, "1");
            }
            if ((cbox_date.Checked == false) && (cbox_type.Checked == true))
            {
                if (cmb_edittype.SelectedIndex > 0)
                {
                    dt = obj.SearchOtherIncome(start, end, cmb_edittype.Text, "2");
                }
                else
                {
            ...
            if ((cbox_date.Checked == true) && (cbox_type.Checked == true))
            {
                if (cmb_edittype.SelectedIndex > 0)
                {
                    dt = obj.SearchOtherIncome(start, end, cmb_edittype.Text, "3");
                }
                else
                {
                    MessageBox.Show("Please select a Type", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void bttn_delete_Click(object sender, EventArgs e)
        {
            ...
            if (DeleteStatus == "1")
            {
                ...
                {
                    obj.DeleteIncome(id);
                    MessageBox.Show("Income sucessully deleted ", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    isupdate = 0;
                    clear();
                    cmb_newtype.Focus();
                ...
                {
                    MessageBox.Show("Please select Expence for delete ", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            else
            {

                MessageBox.Show("You Do Not Have The Permission To Delete Other Income", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void bttn_save_MouseMove(object sender, MouseEventArgs e)
        {
        ...
        {
            IncomeExpenseType ob = new IncomeExpenseType(1);
            ob.ShowDialog();
            DataSet ds = obj.GetOtherIncomeType();
            cmb_newtype.DataSource = ds.Tables[0];
            cmb_newtype.DisplayMember = "Name";
