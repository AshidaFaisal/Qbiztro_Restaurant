            ...
            if (lblWaiter.Tag.ToString() != "0" && !(lblWaiter.Tag.ToString().Contains("Waiter")))
            {
                ...
                //{
                    ...
                    for (int k = 0; k < dgvKOT.Rows.Count; k++)
                    {
                        ...
                        ObjCode.InsertKotDetail(msg, dgvKOT[4, k].Value.ToString(), dgvKOT[1, k].Value.ToString(), dgvKOT[5, k].Value.ToString(), dgvKOT[2, k].Value.ToString(),"0","0","0","0");
    ...
    btnTable_Click(sender, e);
        ...
        
        {
            if (dtKOT.Rows.Count > 0)
            {
                ...
                e.Graphics.DrawString("Waiter: " + dtKOT.Rows[0]["Waiter"].ToString() + ",      Table : " + dtKOT.Rows[0]["TableName"].ToString(), font10, sBrush, 0, y);
        ...
        else
        {
            if (dgvBill.Rows.Count > 0)
            {
                ...
                string msg1 = ObjCode.InsertInvoiceMaster(txtInvoiceNumber.Text, Convert.ToDateTime(globalvariable.StoreDate), "0", "0", "0", status, txtItemTotal.Text, combliment, txtDiscount.Text, txtNettAmount.Text, "0", "0", user, txt_PackCharge.Text, billtype, Convert.ToDateTime(globalvariable.StoreDate)
                    , billingclass.paymode, billingclass.cashamt.ToString(), billingclass.cardamt.ToString(), "Touch","0.00");   // Convert.ToDateTime(globalvariable.StoreDate)   Convert.ToDateTime(lblDate.Text)         
                if (combliment == 1)
                {
                ...
                else
                {
                    for (int i = 0; i < dgvBill.Rows.Count; i++)
                    {
                        ObjCode.InsertInvoiceDetails(msg1, dgvBill[3, i].Value.ToString(), dgvBill[1, i].Value.ToString(), dgvBill[4, i].Value.ToString(), dgvBill[2, i].Value.ToString(), dgvBill[5, i].Value.ToString(), lblTable.Tag.ToString(), lblWaiter.Tag.ToString()=="......"?"":lblWaiter.Tag.ToString(),lblToken.Tag.ToString(),"0","0","0","0");
                    }
                    if (lblTable.Tag.ToString() != "0")
                    {
                        ...
                        else
                        {
                           // int compliment = 0;
                            ...
                            else
                            {
                                ...
                                for (int i = 0; i < dgvKOT.Rows.Count; i++)
                                {
                                    ObjCode.InsertInvoiceDetails(msg, dgvBill[4, i].Value.ToString(), dgvBill[1, i].Value.ToString(), dgvBill[5, i].Value.ToString(), dgvBill[2, i].Value.ToString(), "0", lblTable.Tag.ToString(), lblWaiter.Tag.ToString() == "......" ? "" : lblWaiter.Tag.ToString(), lblToken.Tag.ToString(), "0", "0", "0","0");
                                }
