...
namespace BisCarePosEdition
{
    public partial class DiscountReport : Form
    {
        ...
        private void btn_viewreport_Click(object sender, EventArgs e)
        {
            try
            {
                FrmDiscountReport ob = new FrmDiscountReport(dtp_start.Value, dtp_end.Value);
            ob.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void Doc_PrintPageCounterBill(object sender, PrintPageEventArgs e)
        {
           ...
           float xamount = 95;
            ...
            if(dtprintfeatures.Rows[0]["address1"].ToString()=="1")
            {
            e.Graphics.DrawString(CenterString(dtprintfeatures.Rows[0]["address1"].ToString(),25), font20, new SolidBrush(Color.Black), 0, y);
            lineHeight = font10.GetHeight(e.Graphics);
            y += lineHeight;
            }
            if (dtprintfeatures.Rows[0]["address2"].ToString() == "1")
            {
            e.Graphics.DrawString(CenterString(dtprintfeatures.Rows[0]["address2"].ToString(),35), font20, new SolidBrush(Color.Black), 0, y);
            lineHeight = font10.GetHeight(e.Graphics);
            y += lineHeight;
            }
            if (dtprintfeatures.Rows[0]["address3"].ToString() == "1")
            {
                e.Graphics.DrawString(CenterString(dtprintfeatures.Rows[0]["address3"].ToString(), 55), font10, new SolidBrush(Color.Black), 0, y);
                lineHeight = font10.GetHeight(e.Graphics);
                y += lineHeight;
            }
