...
namespace BisCarePosEdition
{
    public partial class SearchBill : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                queryStr = "SELECT max( dbo.tbInvoice.InvoiceNo)as InvoiceNo ,max( dbo.tbInvoice.TotalAmount)as TotalAmount,max( dbo.tbInvoice.date)as date , max(dbo.tbInvoice.Status)as Status ,max( dbo.tbInvoice.CustomerName)as CustomerName ,max(dbo.tbInvoiceDetails.kotId) as kotId,max( dbo.tbInvoiceDetails.TableId)as TableId, max(dbo.tbInvoiceDetails.WaiterId) as WaiterId ";
