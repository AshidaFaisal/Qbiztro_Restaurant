...
{
    public partial class ProductDamageEntry : Form
    {
        public ProductDamageEntry()
        {
            InitializeComponent();
            lb_quantity.Text = "";
        }
       ...
       string  Productstatus = "0";
        private void StockEntry_Load(object sender, EventArgs e)
        {
          try
          {
            ...
            dt1 = obj.GetFormUserRights(File.ReadAllText("user.txt"), "ProductStock");
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {                        
                            ...
                            gv_stock[1, rowindex].Value = cmb_itemcode.Text;
                            ...
                            gv_stock[8, rowindex].Value = txtRemarks.Text;
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            if (SaveStatus == "1")
            {
                    ...
                    if (rbtnNormal.Checked == true && Convert.ToDouble(cboxCustomer.SelectedValue) > 0 || Rb_OldStock.Checked==true)
                    {
                        if (txtDealerRefNo.Text != "" || Rb_OldStock.Checked == true)
                        {
                            if (Convert.ToDouble(cboxPurchaser.SelectedValue) > 0)
                            {
                                ...
                                if (stat == "0")
                                {
                                    //string Sid = obj.InsertStockEntryMaster(, txt_totala.Text, txtPayingAmt.Text, txtDiscount.Text, txtRemarks.Text, Convert.ToDateTime(dtp_stockdate.Value), txtTinNo.Text, txtDealerRefNo.Text, cboxPurchaser.SelectedValue.ToString(), purchasetype.ToString(), "1", "0", txtPayable.Text);
                                    string Sid = obj.InsertProductDamageMaster(Convert.ToDateTime(dtp_stockdate.Value), cboxPurchaser.SelectedValue.ToString(),"1", "0");
                                    for (int i = 0; i < gv_stock.Rows.Count; i++)
                                    {
                                        ...
                                        obj.InsertProductDamageDetails(Sid, gv_stock[9, i].Value.ToString(), gv_stock[5, i].Value.ToString(), gv_stock[8, i].Value.ToString(), "1", "0");

                                    }
                                }
                                MessageBox.Show("Product Damage Details Saved Sucessfully ", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                reset();
                               cmb_itemname.Focus();
                            }
                            else
                            {
                                MessageBox.Show("Please select User", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                cboxPurchaser.Focus();
                            }
                ...
                else
                {
                    ...
                    MessageBox.Show("Please add Items for Product Damage Entry", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    cmb_itemname.Focus();
                }
            }
            else
            {
                ...
                MessageBox.Show("You Do Not Have The Permission To Save Product Damage Entry", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            try{
                DamageEntrySearch s = new DamageEntrySearch();
            s.ShowDialog();
            string id = s.id;
            stockid = id;
                if (s.dr == DialogResult.OK)
            {
                ...
                dtm = obj.GetProductDamageMaster(id);
                if (dtm.Rows.Count > 0)
                {
                    ...
                    cboxPurchaser.SelectedValue = Convert.ToDouble(dtm.Rows[0]["UserId"]);
                ...
                dt = obj.GetProductDamageDetails(id);
                int i;
                for (i = 0; i < dt.Rows.Count; i++)
                {
                  ...
                  //  gv_stock[7, i].Value = (Convert.ToDateTime(dt.Rows[i]["Date"])).ToShortDateString();
                    gv_stock[8, i].Value = dt.Rows[i]["Remarks"].ToString();
                    gv_stock[9, i].Value = dt.Rows[i]["Item_ID"].ToString();
            ...
            try
            {
                if (UpdateStatus == "1")
                {
                            ...
                            if (txtDealerRefNo.Text != "" || Rb_OldStock.Checked == true)
                            {
                                if (Convert.ToDouble(cboxPurchaser.SelectedValue) > 0)
                                {
                                    ...
                                    if (stat == "0")
                                    {
                                        ...
                                        obj.InsertProductDamageMaster(Convert.ToDateTime(dtp_stockdate.Value), cboxPurchaser.SelectedValue.ToString(), "0", stockid);
                                        decimal newInteger;
                                        NumStat = 0;
                                        ...
                                        for (int i = 0; i < gv_stock.Rows.Count; i++)
                                        {
                                            if((!decimal.TryParse(gv_stock[5, i].FormattedValue.ToString(), out newInteger) || newInteger < 0))
                                            //if ((!decimal.TryParse(gv_stock[4, i].FormattedValue.ToString(), out newInteger) || newInteger < 0) || (!decimal.TryParse(gv_stock[5, i].FormattedValue.ToString(), out newInteger) || newInteger < 0))
                                                NumStat = 1;
                                        }
                                        if (NumStat == 1)
                                        ...
                                        else
                                        {
                                            for (int i = 0; i < gv_stock.Rows.Count; i++)
                                            {
                                                ...
                                                obj.InsertProductDamageDetails(stockid, gv_stock[9, i].Value.ToString(), gv_stock[5, i].Value.ToString(), gv_stock[8, i].Value.ToString(), "1", txtsid.Text);
                                                    
                                                
                                            }
                                            MessageBox.Show("Product Damage Details Updated Successfully", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                            reset();
                                        }
                                    }
                                }
                    ...
                    else
                    {
                        ...
                        MessageBox.Show("Please add Items for Product Damage", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        cmb_itemname.Focus();
                    }
                ...
                else
                {
                    ...
                    MessageBox.Show("You Do Not Have The Permission To Update  Product Damage", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (DeleteStatus == "1")
            {
                if (gv_stock.Rows.Count >= 1)
                {
                    ...
                    int stat = obj.DeleteDamageEntry(stockid);
                     if (stat==0)
                     {
            ...
            else
            {
                ...
                MessageBox.Show("You Do Not Have The Permission To Delete Product Damage", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void gv_stock_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if(gv_stock.RowCount>0)
            {
             ...
             txtRemarks.Text = gv_stock.CurrentRow.Cells[8].Value.ToString();
