    ...
    if (dgvKOT.Rows.Count > 0)
    {
        ...
        if (lblTable.Tag.ToString() != "0")
        {
            if (lblWaiter.Tag.ToString() != "0" && !(lblWaiter.Tag.ToString().Contains("Waiter")))
            {
                ...
                //{
                    ...
                    //for (int i = 0; i < li.Count; i++)
                    //{
                string msg = ObjCode.InsertKotMaster("Cash", "", "", txtItemTotal.Text, "0", lblTable.Tag.ToString(), lblWaiter.Tag.ToString(), "1", "0", DateTime.Now, dgvKOT[6, 0].Value.ToString(), "0", txt_PackCharge.Text, txtInvoiceNumber.Text);//li[i]
        ...
        else
        {
            if (lblToken.Tag.ToString() != "0")
            {
                ...
                string msg = ObjCode.InsertKotMaster("Take away", "", "", txtItemTotal.Text, "0", "0", "0", "2", lblToken.Tag.ToString(), DateTime.Now, dgvKOT[6, 0].Value.ToString(), "1", txt_PackCharge.Text, txtInvoiceNumber.Text);
                    for (int k = 0; k < dgvKOT.Rows.Count; k++)
                    {
                        //if (li[i] == this.dgvKOT[6, k].Value.ToString())
                        //{
        ...
        else
        {
            if (dgvBill.Rows.Count > 0)
            {
                ...
                string msg1 = ObjCode.InsertInvoiceMaster(txtInvoiceNumber.Text, Convert.ToDateTime(globalvariable.StoreDate), "0", "0", "0", status, txtItemTotal.Text, combliment, txtDiscount.Text, txtNettAmount.Text, "0", "0", user, txt_PackCharge.Text, billtype, Convert.ToDateTime(globalvariable.StoreDate)
                    , billingclass.paymode, billingclass.cashamt.ToString(), billingclass.cardamt.ToString(), "Touch");   // Convert.ToDateTime(globalvariable.StoreDate)   Convert.ToDateTime(lblDate.Text)         
                if (combliment == 1)
                {
